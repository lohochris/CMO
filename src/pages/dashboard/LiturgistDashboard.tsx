...
export default function LiturgistDashboard() {
  const [isExecutiveUnlocked, setIsExecutiveUnlocked] = useState<boolean>(() => {
    if (typeof window !== 'undefined') {
      return sessionStorage.getItem('liturgist_session_unlocked') === 'true';
    }
    return false;
  });
  ...
  useEffect(() => {
    const queryTerm = memberSearch.trim();
    ...
    const delayDebounce = setTimeout(async () => {
      ...
      try {
        ...
        if (queryTerm) {
          query = query.or(`full_name.ilike.%${queryTerm}%,official_member_id.ilike.%${queryTerm}%`);
        }
        ...
        console.error("Error searching members in LiturgistDashboard:", err);
      } finally {
    ...
    try {
      ...
      const { error: insertErr } = await supabase
        ...
        .insert([{
          ...
          custom_assignee_name: fellowshipAssigneeType === 'family' ? `${fellowshipFamily} Family` : assigneeName,
...
Notes: ${fellowshipNotes.trim() || 'No special instructions'}`;
  ...
  const handleVerifyPin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isValid) {
        ...
        sessionStorage.setItem('liturgist_session_unlocked', 'true');
        setPinInput("");
      } else {
  ...
  const handleLockDashboard = () => {
    ...
    sessionStorage.removeItem('liturgist_session_unlocked');
  };
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 bg-[#002520] p-6 rounded-xl border border-[#ffd700]/20 shadow-xl">
          ...
          {isExecutiveUnlocked && (
            <button
              ...
              className="bg-[#001a16] hover:bg-[#ffd700]/10 text-[#ffd700] border border-[#ffd700]/30 px-3 py-2 rounded text-sm font-semibold transition-colors flex items-center gap-2 shrink-0 cursor-pointer self-stretch md:self-auto justify-center"
        ...
        {!isExecutiveUnlocked ? (
          <div className="flex flex-col items-center justify-center py-16 px-4 bg-[#001411] border border-[#ffd700]/20 rounded-lg max-w-md mx-auto text-center space-y-6 my-8 shadow-xl">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400 mt-1">Please enter your Authorization PIN to unlock administrative spiritual assignments, trackers, and calendar guidelines.</p>
            </div>
            <form onSubmit={handleVerifyPin} className="w-full space-y-4">
              ...
              <button type="submit" disabled={isVerifyingPin || pinInput.length < 4} className="w-full bg-[#ffd700] hover:bg-[#e6c200] text-[#001a16] font-bold py-2.5 rounded transition-colors disabled:opacity-50 cursor-pointer">
                {isVerifyingPin ? "Verifying..." : "Unlock Liturgy Workspace"}
              </button>
            </form>
          </div>
        ) : (
          <Tabs defaultValue="duties" className="w-full">
            <TabsList className="bg-[#002520] border border-[#ffd700]/20 w-full justify-start p-1 flex-wrap h-auto gap-1 mb-6">
              <TabsTrigger value="duties" className="data-[state=active]:bg-[#ffd700] data-[state=active]:text-[#001a16] text-[#ffd700] cursor-pointer px-4 py-2 text-sm font-semibold rounded">
                Fellowship & Spiritual Duties
              </TabsTrigger>
              ...
              <TabsTrigger value="calendar" className="data-[state=active]:bg-[#ffd700] data-[state=active]:text-[#001a16] text-[#ffd700] cursor-pointer px-4 py-2 text-sm font-semibold rounded">
                Calendar & Guidelines
              </TabsTrigger>
            </TabsList>

            <TabsContent value="duties" className="space-y-6">
              {/* Thursday Fellowship & Spiritual Activities Calendar Table */}
                                ...
                                <div className="flex justify-center items-center gap-2">
                                  {assignment.status !== 'Completed' && (
                                    <Button
                                      ...
                                      className="bg-emerald-500/10 hover:bg-emerald-600 hover:text-white border border-emerald-500/30 text-emerald-400 text-xs px-2.5 py-1 h-auto cursor-pointer"
                                    >
                                      Mark Completed
                                    </Button>
                                  )}
              ...
              <Card className="bg-[#002520] border border-[#ffd700]/10 p-6 rounded-xl shadow-lg">
                ...
                <form onSubmit={handleAddFellowshipAssignment} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2">
                    <Button type="submit" disabled={fellowshipLoading} className="w-full bg-[#ffd700] hover:bg-[#ffc700] text-[#001a16] font-bold cursor-pointer">
              ...
              <Card className="bg-[#002520] border border-[#ffd700]/10 p-6 rounded-xl shadow-lg">
                ...
                {!activeMeetingId ? (
                  <form onSubmit={handleInitializeAttendance} className="flex flex-col sm:flex-row gap-4 items-end">
                    ...
                    <Button type="submit" className="bg-[#ffd700] hover:bg-[#ffc700] text-[#001a16] font-bold px-6 h-10 cursor-pointer">
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center bg-[#001a16] p-3 rounded-lg border border-[#ffd700]/10">
                      ...
                      <Button 
                        ...
                        className="bg-red-500/10 hover:bg-red-600 hover:text-white border border-red-500/30 text-red-400 text-xs px-3 py-1 h-auto cursor-pointer"
                      >
                        Cancel
                      </Button>
                    </div>
                    ...
                    <div className="overflow-x-auto max-h-[300px] border border-[#ffd700]/10 rounded-lg custom-scrollbar">
                      <table className="w-full text-left text-sm border-collapse">
                        <thead>
                    ...
                    <Button
                      ...
                      className="w-full bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white font-bold h-10 shadow-lg cursor-pointer"
                    >
            ...
            <TabsContent value="calendar" className="space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <Card className="bg-[#002520] border border-[#ffd700]/10 p-6 rounded-xl shadow-lg lg:col-span-1">
                  <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
                  ...
                  <form onSubmit={handleAddGuide} className="flex flex-col gap-4">
                    ...
                    <Button type="submit" className="bg-[#ffd700] hover:bg-[#ffc700] text-[#001a16] font-bold mt-1 cursor-pointer">
                      Publish Guide
                    </Button>
                  </form>
                </Card>

                {/* Liturgical Guide Feed */}
                <Card className="bg-[#002520] border border-[#ffd700]/10 p-6 rounded-xl shadow-lg lg:col-span-2">
                  <h2 className="text-xl font-bold text-white flex items-center gap-2 mb-4">
                    <Bookmark className="w-5 h-5 text-[#ffd700]" />
                    Liturgical Calendar & Guide Feed
                  </h2>
                    ...
                    {guides.map((g) => (
                      <div key={g.id} className="bg-[#001a16] border border-[#ffd700]/10 p-4 rounded-lg relative">
                        <button
                          ...
                          className="absolute top-2.5 right-3 text-red-400 hover:text-red-500 text-xs font-semibold cursor-pointer"
                        ...
                        <div className="flex items-center gap-2 mb-1.5">
                          <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider ${
                            g.type === 'Feast Day' ? 'bg-yellow-500/20 text-yellow-400 border border-yellow-500/30' :
                            g.type === 'Rehearsal Notice' ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30' :
                        ...
                        <h4 className="text-white font-bold text-sm uppercase tracking-tight">{g.title}</h4>
                        <p className="text-gray-300 text-xs leading-relaxed mt-2">{g.content}</p>
                      </div>
                    ))}
                    {guides.length === 0 && (
                      <p className="text-gray-400 text-center py-10 italic text-sm">No guidelines or guides published yet.</p>
                    )}
                  </div>
                </Card>
              </div>
