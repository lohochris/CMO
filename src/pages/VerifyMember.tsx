...
export const VerifyMember: React.FC = () => {
  ...
  useEffect(() => {
    const runVerification = async () => {
      // Extract query string directly from window.location
      const urlParams = new URLSearchParams(window.location.search);
      let rawId = urlParams.get('id') || urlParams.get('memberId') || urlParams.get('member_id') || '';

      // Fallback for path-based routing (/verify/HCC-CMO-26-003)
      if (!rawId) {
      ...
      // Fallback for hash-based routing (/#/verify?id=HCC-CMO-26-003)
      if (!rawId && window.location.hash && window.location.hash.includes('?')) {
        const hashQuery = window.location.hash.split('?')[1];
        const hashParams = new URLSearchParams(hashQuery);
        rawId = hashParams.get('id') || hashParams.get('memberId') || hashParams.get('member_id') || '';
      }
